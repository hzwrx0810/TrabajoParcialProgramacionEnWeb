	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		try {
			http.authorizeRequests()
					...
					.antMatchers("/cat/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')").and()
					.formLogin().successHandler(successHandler).loginPage("/login").loginProcessingUrl("/login").defaultSuccessUrl("/cat/listar")
